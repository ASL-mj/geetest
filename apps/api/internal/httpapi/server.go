...
func NewRouter(services *service.Services, solve *service.SolveService) http.Handler {
	...
	mux.HandleFunc("GET /admin/v1/api-keys", server.requireAdminAny(server.handleAdminListAPIKeys))
	mux.HandleFunc("GET /admin/v1/calls", server.requireAdminAny(server.handleAdminListCalls))
	mux.HandleFunc("GET /admin/v1/quota-ledger", server.requireAdminAny(server.handleAdminListQuotaLedger))

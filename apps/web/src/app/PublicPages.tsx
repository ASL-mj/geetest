...
export function PublicDocs({ onBack, onStart }: PublicDocsProps) {
  ...
  return <div className="docs-public-shell"><PublicHeader onBack={onBack} onDocs={() => undefined} onStart={onStart} docsActive /><main className="docs-public-main"><aside className="docs-public-sidebar"><p className="public-eyebrow">DOCUMENTATION</p><strong>CaptchaFlow API V1</strong><a className="docs-public-sidebar__active" href="#quickstart">快速开始</a><a href="#authentication">认证方式</a><a href="#request">请求参数</a><a href="#response">响应与错误</a><a href="#limits">调用限制</a><div className="docs-public-sidebar__bottom"><span>准备好开始了吗？</span><button className="inline-link" onClick={onStart} type="button">激活 CDK <ArrowRight aria-hidden="true" size={15} /></button></div></aside><div className="docs-public-content"><div className="docs-public-title"><div><p className="public-eyebrow">CAPTCHA SERVICE / API V1</p><h1>接入文档</h1><p>使用平台 API Key 调用统一的验证码解析接口。</p></div><span className="docs-version">V1 · 当前版本</span></div><article className="docs-public-article" id="quickstart"><h2>快速开始</h2><p>平台负责用户、Key、额度、幂等和调用审计。用户程序只需要向平台地址发起 HTTPS 请求。</p><div className="docs-callout docs-callout--teal"><ShieldCheck aria-hidden="true" size={18} /><span>解析服务凭据只保存在平台后端环境中，不出现在浏览器和用户请求里。</span></div><h3 id="authentication">认证方式</h3><p>在请求头中使用平台发放的 Bearer API Key，并为每次请求设置唯一的幂等键。</p><div className="docs-code"><div><span>cURL</span><button className="docs-copy" onClick={copySnippet} title="复制示例" type="button"><Copy aria-hidden="true" size={15} />复制</button></div><pre><code>{snippet}</code></pre></div><h3 id="request">请求参数</h3><div className="docs-table"><div className="docs-table__row docs-table__head"><span>字段</span><span>类型</span><span>必填</span><span>说明</span></div><div className="docs-table__row"><code>captcha_id</code><span>string</span><span>是</span><span>目标验证码 ID</span></div><div className="docs-table__row"><code>risk_type</code><span>string</span><span>是</span><span>V1 固定为 <code>slide</code></span></div></div><h3 id="response">响应与错误</h3><div className="docs-error-list"><div><span className="http-code http-code--success">200</span><span>解析成功，确认消耗 1 次额度。</span></div><div><span className="http-code http-code--warning">422</span><span>参数错误，不扣减额度。</span></div><div><span className="http-code http-code--danger">502</span><span>底层异常，预扣额度自动退回。</span></div></div><h3 id="limits">调用限制</h3><p>请求会按用户/CDK 共享额度执行速率与并发检查。重复的 <code>Idempotency-Key</code> 返回首次请求结果，不会重复扣减。</p></article></div></main></div>;
}
